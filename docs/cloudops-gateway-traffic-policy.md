...
**2026-06-30 canary observability 终验：PASS**

```text
触发方式:
  patch Rollout pod template annotation 生成新 ReplicaSet

canary 阶段:
  Rollout phase: Paused
  VirtualService stable/canary: 75/25
  observability canary_stage.stage: canary_25
  istio_metrics.request_rate_rps: present
  istio_metrics.p95_latency_ms: present
  by_destination: present

回稳阶段:
  Rollout phase: Healthy
  VirtualService stable/canary: 100/0
  observability canary_stage.stage: stable
  by_destination: stable/canary labels present

circuit breaker:
  DestinationRule stable/canary: present
  /traffic destination_rules: present
```

遗留说明：当前集群中的 `cloudops-cicd` 仍未部署 snapshot ID 修复版，因此 `POST /records/snapshot` 仍会复用旧 ID `...snapshot-20260625154014`，且 Argo CD `sync=Unknown` 时 snapshot status 会记录为 failed。代码修复已在 `cloudops-platform` main，需重新部署 `cloudops-cicd` 后复验 snapshot 唯一 ID。
